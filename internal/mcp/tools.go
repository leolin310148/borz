...
var snapshotTool = mcp.NewTool("browser_snapshot",
	...
	mcp.WithNumber("limit", mcp.Description("Maximum number of snapshot content lines; omitted refs are not returned")),
	mcp.WithString("selector", mcp.Description("CSS selector for the snapshot subtree. If it is invalid CSS or matches no root, borz falls back to a case-insensitive keyword match across tag/role/name/xpath/attributes. Combine with role for precision.")),
	...
	mcp.WithBoolean("showRefs", mcp.Description("Human visual-debugging override for the live-page ref overlay. Agents should omit this (or pass false) unless the user explicitly asks to see the overlay. Omit to use ~/.borz/settings.json (visible when unset). Refs are always returned and actionable.")),
...
var clearRefsTool = mcp.NewTool("browser_clear_refs",
	mcp.WithDescription("Remove snapshot ref boxes and numbers from the live page without taking another snapshot or invalidating the latest actionable refs."),
	tabParam(),
)
